...
class MPCController:
    def __init__(self, dt=0.05, N=50):
        ...
        self.Q = np.diag([2.0, 2.0, 0.5, 1.0])  # Increased position tracking
        self.R = np.diag([0.05, 0.05])           # Control weights (steering, throttle)
        ...
        while frontier:
            ...
            for node in self.topology:
                if node['waypoint'] == current:
                    next_waypoints.extend(node['connections'])
                    break
...
class CarEnv:
    ...
    #         return self.get_reference_trajectory(current_state)
    ...
    #     for i in range(current_idx, len(self.global_path)):
    ...
    #         # Calculate desired speed based on curvature
    #         next_idx = min(idx + 1, len(self.global_path) - 1)
    #         if idx < len(self.global_path) - 1:
    #             dx = self.global_path[next_idx].transform.location.x - wp.transform.location.x
    #             dy = self.global_path[next_idx].transform.location.y - wp.transform.location.y
    #             curvature = abs(np.arctan2(dy, dx))
    #             target_speed = max(5.0, 20.0 - 15.0 * curvature)  # Reduce speed in curves
    #         else:
    #             target_speed = 5.0  # Slower near the end

    #         waypoints.append([
    #             wp.transform.location.x,
    #             wp.transform.location.y,
    #             wp.transform.rotation.yaw * np.pi / 180.0,
    #             target_speed
    #         ])
            ...
            if self.global_path:
                ...
                for i in range(len(self.global_path)-1):
                    ...
                    self.world.debug.draw_line(
                        ...
                        next_wp + carla.Location(z=0.5),
                        thickness=0.5,  # Thicker line
                        color=carla.Color(b=0, g=0, r=255),  # Pure blue
                        life_time=20.0,
                        persistent_lines=True
                    )
    ...
    def step(self, action):
        ...
        try:
            ...
            try:
                ...
                reference_trajectory = self.get_reference_from_global_path(state)
