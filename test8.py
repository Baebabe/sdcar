...
class MPCController:
    def __init__(self, dt=0.05, N=40):
        ...
        self.Q = np.diag([15.0, 15.0, 5.0, 2.0])  # Higher yaw weight
        self.R = np.diag([0.05, 0.1])  # Smoother steering
    ...
    def _setup_optimization(self):
        ...
        try:
            ...
            beta = ca.atan(0.5*ca.tan(steer))  # Simple slip model
            rhs = ca.vertcat(
                v * ca.cos(yaw + beta),
                v * ca.sin(yaw + beta),
                v * ca.tan(steer) / self.L * ca.cos(beta),
                5*throttle - 0.2*v**2 - 0.1*v  # Improved resistance model
            )
    ...
    def solve(self, current_state, reference_trajectory, detected_objects):
        """ Solve MPC optimization problem with enhanced constraints and fallback """
        try:
            ...
            # Add path boundary constraints
            for k in range(self.N+1):
                if k < len(reference_trajectory):
                    # Add soft constraints for path deviation (2m lateral tolerance)
                    x = self.X[0, k]
                    y = self.X[1, k]
                    ref_x = reference_trajectory[k, 0]
                    ref_y = reference_trajectory[k, 1]
                    
                    # Constraint for x and y deviation
                    g.append(x - ref_x)
                    g.append(y - ref_y)
                    lbg.extend([-2.0, -2.0])
                    ubg.extend([2.0, 2.0])
            ...
            if np.any(self.x0):
                self.x0 = self._shift_initial_guess(self.x0)
...
class GlobalPathPlanner:
    ...
    def smooth_path(self, path):
        """Apply cubic spline smoothing to the path"""
        if len(path) < 4:
            return path
        ...
        # Create cumulative distance parameter
        dist = np.cumsum(np.sqrt(np.ediff1d(x, to_begin=0)**2 + np.ediff1d(y, to_begin=0)**2))
        
        # Fit splines
        spl_x = CubicSpline(dist, x)
        spl_y = CubicSpline(dist, y)
        
        # Resample at finer intervals
        fine_dist = np.linspace(0, dist[-1], 2*len(dist))
        smooth_x = spl_x(fine_dist)
        smooth_y = spl_y(fine_dist)
...
class CarEnv:
    ...
    def get_reference_from_global_path(self, current_state):
        # Find nearest waypoint with lookahead
        current_pos = np.array([current_state[0], current_state[1]])
        lookahead = 8  # Increased lookahead for better anticipation
        
        # Find closest waypoint ahead of vehicle
        closest_idx = self.current_path_index
        min_angle_diff = float('inf')
        
        for i in range(self.current_path_index, min(len(self.global_path), self.current_path_index+50)):
            wp = self.global_path[i]
            dx = wp.transform.location.x - current_state[0]
            dy = wp.transform.location.y - current_state[1]
            path_angle = np.arctan2(dy, dx)
            vehicle_angle = current_state[2]
            angle_diff = abs(path_angle - vehicle_angle)
        ...
        # Create reference with curvature-based speed
        reference = []
        for i in range(closest_idx, min(closest_idx + self.mpc.N, len(self.global_path)-1)):
            wp = self.global_path[i]
            
            # Calculate curvature
            if i > closest_idx and i < len(self.global_path)-1:
                x_prev = self.global_path[i-1].transform.location.x
                y_prev = self.global_path[i-1].transform.location.y
                x_next = self.global_path[i+1].transform.location.x
                y_next = self.global_path[i+1].transform.location.y
                curvature = self.calculate_curvature(x_prev, y_prev, 
                                                    wp.transform.location.x, wp.transform.location.y,
                                                    x_next, y_next)
            else:
                curvature = 0
                
            # Speed profile based on curvature
            target_speed = max(5.0, 20.0 - 15*abs(curvature))
    ...
    def calculate_curvature(self, x1, y1, x2, y2, x3, y3):
        """Calculate curvature from three points"""
        dx1, dy1 = x2-x1, y2-y1
        dx2, dy2 = x3-x2, y3-y2
        
        den = np.sqrt((dx1**2 + dy1**2)*(dx2**2 + dy2**2)*(dx1*dy2 - dx2*dy1)**2)
        if den < 1e-5:
            return 0
            
        return (dx1*dy2 - dx2*dy1) / den
