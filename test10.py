...
class MPCController:
    def __init__(self, dt=0.05, N=30):
        ...
        self.bounds = {
            ...
            'min_ttc': 5.0,    # Minimum Time-to-Collision threshold in seconds
            'min_following_distance': 3.0  # Minimum following distance in meters
        }
        
        self.max_steer_angle = np.deg2rad(30)
        
        # Safety parameters with enhanced weights
        self.safety_distance = 3.0  # Base safety distance in meters
        self.object_weights = {
            'person': 5.0,      # Highest priority
            'vehicle': 4.0,
            'bicycle': 4.5,
            'motorcycle': 4.5,
            'generic': 3.0      # Default weight for unknown objects
        }
        
        # Path projection parameters
        self.path_lookahead = 30.0  # meters
        self.path_resolution = 0.5   # meters
        ...
        # Store reference path for frenet conversion
        self.global_path = None  # Should be set after initialization
    
    def get_dynamic_safety_distance(self, current_speed: float) -> float:
        """
        Calculate safety distance based on current velocity.
        
        Args:
            current_speed: Current velocity in m/s
            
        Returns:
            float: Dynamic safety distance in meters
        """
        time_headway = 1.5  # seconds
        return max(
            self.bounds['min_following_distance'],
            current_speed * time_headway
        )
    
    def cartesian_to_frenet(self, x: float, y: float, path_points: np.ndarray) -> Tuple[float, float]:
        """
        ...
        Returns:
            Tuple[float, float]: (s, d) coordinates in Frenet frame
        """
        # Find closest point on path
        path_array = np.array(path_points)
        distances = np.sqrt(np.sum((path_array - np.array([x, y]))**2, axis=1))
        closest_idx = np.argmin(distances)
        
        # Calculate s (distance along path)
        s = np.sum(np.sqrt(np.sum(np.diff(path_array[:closest_idx+1], axis=0)**2, axis=1)))
        
        # Calculate d (lateral distance)
        if closest_idx < len(path_points) - 1:
            path_heading = np.arctan2(
                path_points[closest_idx+1][1] - path_points[closest_idx][1],
                path_points[closest_idx+1][0] - path_points[closest_idx][0]
            )
            d = distances[closest_idx] * np.sign(
                np.cross(
                    [np.cos(path_heading), np.sin(path_heading)],
                    [x - path_points[closest_idx][0], y - path_points[closest_idx][1]]
                )
            )
        else:
    ...
    def is_object_in_path(self, obj: Dict, current_state: np.ndarray) -> bool:
        """
        Check if object lies within projected path corridor.
        
        Args:
            obj: Dictionary containing object information
            current_state: Current vehicle state [x, y, yaw, v]
        ...
        ego_s, ego_d = self.cartesian_to_frenet(
            current_state[0], 
            current_state[1],
            self.global_path
        )
        obj_s, obj_d = self.cartesian_to_frenet(obj_x, obj_y, self.global_path)
        
        # Check if object is ahead and within lane boundaries
        half_lane = self.bounds['lane_width'] / 2
        return (obj_s > ego_s and  # Object is ahead
                obj_s - ego_s < self.path_lookahead and  # Within lookahead distance
                abs(obj_d) < half_lane)  # Within lane boundaries
    
    def should_brake_for_object(self, obj: Dict, current_speed: float) -> bool:
        """
        Determine if braking is needed based on Time-to-Collision.
        ...
        Returns:
            bool: True if braking is needed, False otherwise
        """
        # Extract object information
        longitudinal_distance = obj.get('longitudinal_distance', 
            np.hypot(obj['position'][0], obj['position'][1]))
        obj_speed = obj.get('speed', 0.0)
        
        # Calculate relative speed
        relative_speed = current_speed - obj_speed
        
        # If we're not approaching the object, no need to brake
        if relative_speed <= 0:
            return False
        ...
        # Check if TTC is below threshold
        return ttc < self.bounds['min_ttc']
    ...
    def solve(self, current_state, reference_trajectory, detected_objects):
        """ 
        Solve MPC optimization problem with enhanced obstacle avoidance and safety features

        Args:
            current_state: Current vehicle state [x, y, yaw, v]
            reference_trajectory: Desired trajectory points
            detected_objects: List of detected objects with position, type, and speed info
        """
        try:
            ...
            # Handle reference trajectory dimensions
            ...
            # Verify and adjust parameter vector size if needed
            ...
            # Dynamic safety distance based on current speed
            dynamic_safety_dist = self.get_dynamic_safety_distance(current_state[3])

            # State bounds with safety considerations
            for i in range(self.N + 1):
                lbx[i * self.n_states:(i + 1) * self.n_states] = [
                    -np.inf, -np.inf, -2*np.pi, 
                    self.bounds['v_min']  # Minimum velocity
                ]
                ubx[i * self.n_states:(i + 1) * self.n_states] = [
                    np.inf, np.inf, 2*np.pi,
                    self.bounds['v_max']  # Maximum velocity
                ]

                # Adjust velocity bounds based on obstacle proximity
                for obj in detected_objects:
                    if self.is_object_in_path(obj, current_state):
                        if self.should_brake_for_object(obj, current_state[3]):
                            # Reduce maximum velocity for safety
                            ubx[i * self.n_states + 3] = min(
                                ubx[i * self.n_states + 3],
                                obj.get('speed', 0.0) * 0.9  # 90% of obstacle speed
                            )
            ...
            def calculate_obstacle_cost(opt_vars):
                """Calculate cost term for obstacle avoidance"""
                states = opt_vars[:(self.N+1)*self.n_states].reshape((self.N+1, self.n_states))
                total_cost = 0.0

                for obj in detected_objects:
                    if not self.is_object_in_path(obj, current_state):
                        continue

                    obj_x, obj_y = obj['position'][0], obj['position'][1]
                    obj_weight = self.object_weights.get(obj.get('type', 'generic'), 
                                                       self.object_weights['generic'])
                    ...
                    for k in range(self.N):
                        ...
                        # Exponential cost that grows quickly as distance decreases
                        cost = obj_weight * np.exp(-dist_sq / (2 * dynamic_safety_dist**2))

                        # Additional cost for being too close
                        if np.sqrt(dist_sq) < dynamic_safety_dist:
                            cost *= 10.0  # Penalty multiplier for safety violation
            ...
            try:
                ...
                sol = self.solver(
                    ...
                    ubg=ubg,
                    p=param_vector,
                    obstacle_cost_fn=calculate_obstacle_cost
                )
    ...
    def solve_with_obstacle_avoidance(self, current_state, reference_trajectory, detected_objects):
        ...
        safety_dist = self.get_dynamic_safety_distance(current_speed)
    
        for obj in detected_objects:
            if self.is_object_in_path(obj, current_state):
                # Use longitudinal distance, not depth
                long_dist = self.calculate_longitudinal_distance(obj, current_state)

                if long_dist < safety_dist or self.should_brake_for_object(obj, current_speed):
                    # Progressive braking based on urgency
                    brake_intensity = min(1.0, (safety_dist - long_dist)/safety_dist)
                    return np.array([0.0, -brake_intensity])
