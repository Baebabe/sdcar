...
class SafetyController:
    def __init__(self, parent_actor, world, controller):
        ...
        self.time_to_collision_threshold = 3.0  # seconds
        self.min_safe_distance = 10.0  # meters
        self.emergency_brake_distance = 7.0  # meters

        self.traffic_light_detection_distance = 25.0  # meters
        self.yellow_light_threshold = 2.0  # seconds - time to start slowing for yellow
    ...
    def check_safety(self):
        ...
        try:
            ...
            if not self._check_traffic_lights():
                return  # Stop if red/yellow light detected
    ...
    def _check_traffic_lights(self):
        """Check and respond to traffic lights"""
        try:
            # Get ego vehicle's location and transform
            ego_location = self._parent.get_location()
            ego_transform = self._parent.get_transform()
            ego_forward = ego_transform.get_forward_vector()
            ...
            for traffic_light in traffic_lights:
                ...
                # Check if light is within detection range
                if distance > self.traffic_light_detection_distance:
                    continue
                    
                # Check if light is in front of vehicle
                to_light = light_location - ego_location
                forward_dist = (to_light.x * ego_forward.x + to_light.y * ego_forward.y)
                
                if forward_dist <= 0:  # Light is behind vehicle
                    continue
                
                # Get the trigger volume (box) of the traffic light
                trigger_volume = traffic_light.trigger_volume
                if not trigger_volume:
                    continue
                    
                # Check if our vehicle is within the trigger volume
                vehicle_in_volume = self._is_vehicle_in_box(ego_location, trigger_volume)
                
                if vehicle_in_volume and distance < min_distance:
                    relevant_light = traffic_light
                    min_distance = distance
            
            if relevant_light:
                light_state = relevant_light.state  # In 0.9.8, state is directly accessible
                
                # Debug visualization
                if self.debug:
                    color = self._get_debug_color(light_state)
                    light_location = relevant_light.get_location()
                    self._world.debug.draw_point(
                        light_location,
                        size=0.3,
                        color=color,
                    ...
                    self._world.debug.draw_string(
                        light_location,
                        f'Traffic Light: {light_state} ({min_distance:.1f}m)',
                        color=color,
                        life_time=0.1
                    )
                ...
                if light_state == carla.TrafficLightState.Red:
                    print(f"Red light detected at {min_distance:.1f}m")
                    self._handle_red_light(min_distance)
                    return False
                    
                elif light_state == carla.TrafficLightState.Yellow:
                    current_time = time.time()
                    
                    # Start tracking yellow light timing
                    if self.last_traffic_light != relevant_light:
                        self.yellow_light_start_time = current_time
                        self.last_traffic_light = relevant_light
                    
                    # Decide whether to stop or proceed
                    if self._should_stop_for_yellow(min_distance, current_time):
                        print(f"Stopping for yellow light at {min_distance:.1f}m")
                        self._handle_yellow_light(min_distance)
                        return False
                    
                elif light_state == carla.TrafficLightState.Green:
                    print(f"Green light detected at {min_distance:.1f}m")
                    self.last_traffic_light = None
                    self.yellow_light_start_time = None
                    # Continue normal operation
                    
                self.traffic_light_status = light_state
            
            return True  # No red/yellow lights requiring stop
            
        except Exception as e:
            print(f"Error checking traffic lights: {str(e)}")
            return True  # Conservative approach - continue if error
            
    def _is_vehicle_in_box(self, location, trigger_volume):
        """Check if vehicle location is within the traffic light's trigger volume"""
        try:
            # Get traffic light box extents and transform
            box_location = trigger_volume.location
            box_extent = trigger_volume.extent
            
            # Calculate relative position of vehicle to box center
            rel_pos = location - box_location
            
            # Simple box check
            return (abs(rel_pos.x) <= box_extent.x and
                   abs(rel_pos.y) <= box_extent.y and
                   abs(rel_pos.z) <= box_extent.z)
                   
        except Exception as e:
            print(f"Error in box check: {str(e)}")
            return False
            
            
    def _handle_red_light(self, distance):
        """Handle red light based on distance"""
        if distance < self.emergency_brake_distance:
            self._emergency_stop()
        else:
            brake_intensity = min(0.8, self.emergency_brake_distance / distance)
            control = carla.VehicleControl(
                throttle=0.0,
                brake=brake_intensity,
                hand_brake=False
    ...
    def _handle_yellow_light(self, distance):
        """Handle yellow light based on distance and speed"""
        velocity = self._parent.get_velocity()
        speed = math.sqrt(velocity.x**2 + velocity.y**2)
        
        # Calculate brake intensity based on speed and distance
        brake_intensity = min(0.6, (speed * 0.1) + (self.emergency_brake_distance / distance))
    ...
    def _should_stop_for_yellow(self, distance, current_time):
        """Decide whether to stop for yellow light based on timing and distance"""
        ...
        # Time since light turned yellow
        time_in_yellow = current_time - self.yellow_light_start_time
        
        # If we just started yellow and we're far enough, stop
        if time_in_yellow < self.yellow_light_threshold:
            stopping_distance = (speed ** 2) / (2 * 3.0)  # Assuming 3.0 m/s² deceleration
            return distance > stopping_distance
            
        # If we're close to the intersection and light has been yellow for a while, proceed
        return distance > self.emergency_brake_distance
            
    def _get_debug_color(self, light_state):
        """Get debug visualization color based on traffic light state"""
        if light_state == carla.TrafficLightState.Red:
        ...
        return carla.Color(128, 128, 128)
