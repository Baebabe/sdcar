...
class MPCController:
    def __init__(self, dt=0.1, N=10):
        ...
        self.Q = np.diag([10.0, 10.0, 10.0, 5.0])  # Higher position/yaw weights
        self.R = np.diag([0.2, 0.1])  # Less aggressive control penalty    
        ...
        self.bounds = {
            ...
            'max_lat_acc': 2,  # m/s² (3.0 for aggressive driving)
            'steer_rate_limit': 1.57  # rad/s
        }
    ...
    def _setup_optimization(self):
        ...
        try:
            ...
            beta = ca.atan(ca.tan(steer) * 0.5)  # Slip angle approximation
            rhs = ca.vertcat(
                v * ca.cos(yaw + beta),
                v * ca.sin(yaw + beta),
                v * ca.sin(beta) / (self.L/2),  # Improved yaw dynamics
                2.8*throttle - 0.25*v**2 - 0.1*v  # Enhanced longitudinal dynamics
            )
    ...
    def solve(self, current_state, reference_trajectory, detected_objects):
        """ Solve MPC optimization problem with improved warm-start initialization """
        # Improved warm-start initialization
        if self.x0 is None or len(self.x0) != self.n_vars:
            self.x0 = np.zeros(self.n_vars)
        else:
            # Shift previous solution
            prev_states = self.x0[:(self.N+1)*self.n_states].reshape(-1, self.n_states)
            prev_controls = self.x0[(self.N+1)*self.n_states:].reshape(-1, self.n_controls)
            
            # Linear extrapolation for state initialization
            new_states = np.roll(prev_states, -1, axis=0)
            new_states[-1] = 2*new_states[-2] - new_states[-3]  # Quadratic extrapolation
            
            # Maintain control initialization
            new_controls = np.roll(prev_controls, -1, axis=0)
            new_controls[-1] = new_controls[-2]
            
            self.x0 = np.concatenate([new_states.flatten(), new_controls.flatten()])
        ...
        try:
            ...
            try:
                ...
                if self.solver.stats()['success']:
                    ...
                    # MPC solver failed, use emergency steering
                    print("MPC solver failed to find a solution")
                    # Calculate direction to next waypoint
                    current_yaw = current_state[2]
                    target_vec = reference_trajectory[0][:2] - current_state[:2]
                    target_yaw = np.arctan2(target_vec[1], target_vec[0])
                    yaw_error = target_yaw - current_yaw
                    
                    # Simple P control for emergency steering
                    steer = np.clip(yaw_error * 0.8, -1.0, 1.0)
                    throttle = 0.3  # Reduced speed
                    
                    return np.array([steer, throttle])

            except Exception as e:
                print(f"MPC solver error: {e}")
                # Emergency steering fallback
                current_yaw = current_state[2]
                target_vec = reference_trajectory[0][:2] - current_state[:2]
                target_yaw = np.arctan2(target_vec[1], target_vec[0])
                yaw_error = target_yaw - current_yaw
            ...
            # Emergency steering for any unhandled exceptions
            current_yaw = current_state[2]
            target_vec = reference_trajectory[0][:2] - current_state[:2]
            target_yaw = np.arctan2(target_vec[1], target_vec[0])
            yaw_error = target_yaw - current_yaw
    ...
    def solve_with_obstacle_avoidance(self, current_state, reference_trajectory, detected_objects):
        """Fallback strategy for tight turns"""
        try:
            # Calculate required turning radius
            current_speed = current_state[3]
            target_yaw = reference_trajectory[0,2]
            yaw_error = target_yaw - current_state[2]

            min_turn_radius = current_speed**2 / (self.bounds['max_lat_acc'] + 1e-3)
            required_steer = np.arctan(self.L / min_turn_radius)

            if np.abs(required_steer) > self.bounds['steer'][1]:
                print(f"Emergency turn: {np.degrees(required_steer):.1f}°")
                # Direct geometric control
                steer = np.clip(required_steer, -1.0, 1.0)
                throttle = 0.3 if current_speed < 5 else 0.1
                return np.array([steer, throttle])
...
class CarEnv:
    ...
    def visualize_current_progress(self, current_loc, target_wp):
        ...
        try:
            # Draw predicted path from MPC solution
            if hasattr(self, 'mpc') and self.mpc.x0 is not None:
                states = self.mpc.x0[:(self.mpc.N+1)*4].reshape(-1, 4)
                for i in range(self.mpc.N):
                    p1 = carla.Location(x=states[i][0], y=states[i][1], z=0.5)
                    p2 = carla.Location(x=states[i+1][0], y=states[i+1][1], z=0.5)
                    self.world.debug.draw_line(
                        p1, p2,
                        color=carla.Color(r=255, g=165, b=0),
                        thickness=0.1,
                        life_time=0.1
                    )
            ...
            for i in range(len(self.global_path)-1):
                p1 = self.global_path[i].transform.location + carla.Location(z=0.3)
                p2 = self.global_path[i+1].transform.location + carla.Location(z=0.3)
                self.world.debug.draw_line(
                    p1, p2,
    ...
    def get_reference_from_global_path(self, current_state):
        """Generate velocity-profile-optimized reference trajectory with improved turn handling"""
        try:
            ...
            reference = []
            look_ahead = min(self.mpc.N, len(self.global_path) - self.current_path_index)

            for i in range(look_ahead):
                wp_idx = self.current_path_index + i
                if wp_idx >= len(self.global_path):
                    break

                wp = self.global_path[wp_idx]

                # Calculate curvature for speed adjustment
                if i < look_ahead - 2:
                    x0 = self.global_path[wp_idx].transform.location.x
                    y0 = self.global_path[wp_idx].transform.location.y
                    x1 = self.global_path[wp_idx + 1].transform.location.x
                    y1 = self.global_path[wp_idx + 1].transform.location.y
                    x2 = self.global_path[wp_idx + 2].transform.location.x
                    y2 = self.global_path[wp_idx + 2].transform.location.y

                    curvature = self._calculate_curvature(x0, y0, x1, y1, x2, y2)

                    # Adjust speed based on curvature
                    base_speed = 15.0  # m/s
                    min_speed = 5.0   # m/s
                    curvature_factor = np.exp(-2.0 * curvature)  # Exponential decay
                    target_speed = min_speed + (base_speed - min_speed) * curvature_factor
                else:
                    target_speed = min_speed

                # Add waypoint to reference trajectory
                reference.append([
                    wp.transform.location.x,
                    wp.transform.location.y,
                    np.radians(wp.transform.rotation.yaw),  # Convert to radians
                    target_speed
                ])
            ...
            print(f"Error in reference trajectory generation: {e}")
            return None

    def _calculate_curvature(self, x0, y0, x1, y1, x2, y2):
        """Calculate curvature using three points with improved accuracy"""
        try:
    ...
    def get_next_waypoint(self, current_location):
        """Get next target waypoint with adaptive lookahead"""
        if not self.global_path:
            return None, 0

        # Calculate adaptive lookahead based on speed
        velocity = self.vehicle.get_velocity()
        speed = math.sqrt(velocity.x**2 + velocity.y**2)
        lookahead = max(5.0, speed * 1.5)  # Dynamic lookahead

        # Find closest waypoint
        closest_idx = np.argmin([current_location.distance(wp.transform.location) 
                               for wp in self.global_path])

        # Search ahead for target waypoint
        target_idx = closest_idx
        while target_idx < len(self.global_path)-1:
            dist = current_location.distance(self.global_path[target_idx].transform.location)
            if dist > lookahead:
                break
            target_idx += 1
    ...
    def step(self, action):
        ...
        try:
            ...
            try:
                ...
                mpc_action = self.mpc.solve(current_state, reference_trajectory, [])
            ...
            # Advanced waypoint progression check
            if self.global_path and self.current_path_index < len(self.global_path):
                target_wp = self.global_path[self.current_path_index]
                current_loc = self.vehicle.get_location()
                ...
                target_vector = np.array([
                    target_wp.transform.location.x - current_loc.x,
                    target_wp.transform.location.y - current_loc.y
                ])
                current_yaw = np.radians(self.vehicle.get_transform().rotation.yaw)
                vehicle_vector = np.array([np.cos(current_yaw), np.sin(current_yaw)])
                
                # Calculate angle between vectors
                angle_to_target = np.arccos(
                    np.clip(
                        np.dot(target_vector, vehicle_vector) / 
                        (np.linalg.norm(target_vector) * np.linalg.norm(vehicle_vector)),
                        -1.0, 1.0
                    )
                )
                
                # Progress to next waypoint if close enough and roughly aligned
                if distance_to_target < 2.0 and angle_to_target < np.pi/4:  # Within 45 degrees
