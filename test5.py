...
class MPCController:
    def __init__(self, dt=0.05, N=20):
        ...
        self.Q = np.diag([10.0, 10.0, 5.0, 1.0])  # x, y, yaw, velocity
        self.R = np.diag([0.05, 0.1])   # steering, throttle
        self.g = []  # List to store constraints
        self.lbg = []  # Lower bounds for constraints
        self.ubg = []  # Upper bounds for constraints
        # Vehicle parameters
        ...
        self._add_steering_rate_constraints()
    ...
    def _add_steering_rate_constraints(self):
            """Add steering rate constraints to MPC formulation"""
            for k in range(1, self.N):
                # Steering rate: (steer_k - steer_{k-1})/dt <= max_steer_rate
                self.g.append((self.U[0,k] - self.U[0,k-1])/self.dt)
                self.lbg.append(-self.bounds['max_steer_rate'])
                self.ubg.append(self.bounds['max_steer_rate'])
...
class GlobalPathPlanner:
    ...
    def _add_velocity_to_path(self, path):
        """
        Add velocity information to path waypoints based on both lookahead curve detection
        and sliding window curvature calculation.
        
        Args:
            path: List of waypoints
            
        Returns:
            List of Waypoint objects with velocity information
        """
        waypoints = []
        ...
        for i in range(len(path)):
            ...
            # Look 3 waypoints ahead for better curve detection
            lookahead = min(i + 3, len(path) - 1)
            dx_ahead = path[lookahead].transform.location.x - wp.transform.location.x
            dy_ahead = path[lookahead].transform.location.y - wp.transform.location.y
            lookahead_angle = math.atan2(dy_ahead, dx_ahead)
            
            # Calculate curvature using sliding window
            window_size = 3
            window = path[max(0, i - window_size):min(len(path), i + window_size)]
            
            if len(window) > 2:
                # Get x, y coordinates for window points
                x = [w.transform.location.x for w in window]
                y = [w.transform.location.y for w in window]
                window_curvature = self._calculate_curvature(x, y)
                
                # Combine both curvature measures
                # Using the maximum of lookahead angle and window curvature
                curvature = max(abs(lookahead_angle), abs(window_curvature))
                
                # Calculate speed reduction based on curvature
                speed_reduction = min(15.0 * curvature, 15.0)
                velocity = max(5.0, 20.0 - speed_reduction)
            else:
                # If window is too small, use simpler calculation
                velocity = max(5.0, 20.0 - 15.0 * abs(lookahead_angle))
            
            # Create waypoint with calculated velocity
            waypoints.append(Waypoint(wp.transform, velocity))
        
        return waypoints

    def _calculate_curvature(self, x, y):
        """Calculate curvature using cubic spline interpolation"""
        t = np.arange(len(x))
...
class CarEnv:
    ...
    def get_reference_from_global_path(self, current_state):
        ...
        if self._is_sharp_turn_ahead():
            return self._get_smoothed_turn_reference()
    ...
    def _is_sharp_turn_ahead(self):
        """Detect upcoming 90-degree turns using waypoint angles"""
        lookahead = 5
        if self.current_path_index + lookahead >= len(self.global_path):
            return False
            
        angles = []
        for i in range(lookahead):
            wp1 = self.global_path[self.current_path_index + i]
            wp2 = self.global_path[self.current_path_index + i + 1]
            dx = wp2.transform.location.x - wp1.transform.location.x
            dy = wp2.transform.location.y - wp1.transform.location.y
            angles.append(math.atan2(dy, dx))
            
        angle_changes = np.abs(np.diff(angles))
        return any(angle_changes > math.pi/2.2)  # > ~90 degrees

    def _get_smoothed_turn_reference(self):
        """Generate smoother trajectory for sharp turns"""
        # Add intermediate waypoints using cubic spline
        start_idx = max(0, self.current_path_index - 2)
        end_idx = min(len(self.global_path), self.current_path_index + 5)
        
        x = [wp.transform.location.x for wp in self.global_path[start_idx:end_idx]]
        y = [wp.transform.location.y for wp in self.global_path[start_idx:end_idx]]
        
        t = np.linspace(0, 1, len(x))
        cs_x = CubicSpline(t, x)
        cs_y = CubicSpline(t, y)
        
        new_t = np.linspace(0, 1, 10)  # 10 intermediate points
        smoothed_points = np.vstack([cs_x(new_t), cs_y(new_t)]).T
        
        # Create temporary waypoints with reduced speed
        ref_traj = []
        for point in smoothed_points:
            wp = Waypoint(
                ...
                velocity=10.0  # Reduced speed for turns
            )
            ref_traj.append(wp)
            
        return self._convert_to_mpc_format(ref_traj)
    ...
    def apply_control(self, waypoint, current_state):
        """Improved control for sharp turns"""
        # Calculate target yaw using both waypoint rotation and path direction
        target_rotation = waypoint.transform.rotation
        path_yaw = math.atan2(waypoint.transform.location.y - current_state[1],
                             waypoint.transform.location.x - current_state[0])
        blended_yaw = 0.7*math.radians(target_rotation.yaw) + 0.3*path_yaw

        # Model predictive feed-forward steering
        lookahead_steer = math.atan2(self.L * waypoint.curvature, 1)

        # PID controller
        yaw_error = blended_yaw - current_state[2]
        steer = np.clip(1.2*yaw_error + 0.8*lookahead_steer, -1.0, 1.0)

        # Speed control with curve anticipation
        speed_error = waypoint.velocity - current_state[3]
        throttle = np.clip(0.6*speed_error + 0.2*(waypoint.velocity/20.0), 0, 1)

        return carla.VehicleControl(
            throttle=float(throttle),
            steer=float(steer),
            brake=float(max(0, -speed_error*0.5))
            )
    ...
    def generate_global_path(self):
        ...
        try:
            ...
            if not start_waypoint or not end_waypoint:
                print("Invalid start or end waypoint")
                return False
