...
class MPCController:
    def __init__(self, dt=0.05, N=30):
        ...
        self.Q = np.diag([5.0, 5.0, 5.0, 1.0])  # Increased yaw weight
        self.R = np.diag([0.5, 0.5])  # Reduced control penalty

        
        # Vehicle parameters
        self.L = 2.9  # Wheelbase
        self.bounds = {
            'steer': [-0.785, 0.785],  # 45 degrees in radians
            'throttle': [0, 1.0],
            'v_max': 12.0,
            'v_min': 0.0,
            'max_yaw_rate': 3.14  # Increased yaw rate capacity (180 deg/s)
        }
    ...
    def _setup_optimization(self):
        ...
        try:
            ...
            v * ca.cos(yaw),
            v * ca.sin(yaw),
            (v / self.L) * ca.tan(steer),  # Direct steering input
            throttle - 0.1*v  # Simplified longitudinal dynamics
            )
    ...
    def solve(self, current_state, reference_trajectory, detected_objects):
        """ Solve MPC optimization problem with enhanced fallback mechanism """
        try:
            ...
            if not self._validate_inputs(current_state, reference_trajectory):
                print("Input validation failed, falling back to geometric controller")
                return self.geometric_fallback(current_state, reference_trajectory[0])

            # Process reference trajectory
            reference_trajectory = self._process_reference_trajectory(reference_trajectory)
            ...
            try:
                ...
                if param_vector.size != expected_param_size:
                    print(f"Parameter vector size mismatch. Expected {expected_param_size}, got {param_vector.size}")
                    return self.geometric_fallback(current_state, reference_trajectory[0])
            except ValueError as e:
            ...
            # Set up bounds
            try:
                lbx, ubx = self._setup_variable_bounds()
                lbg, ubg = self._setup_constraint_bounds()
            except Exception as e:
                print(f"Error setting up bounds: {e}")
                return self.geometric_fallback(current_state, reference_trajectory[0])
            ...
            try:
                ...
                if self.solver.stats()['success']:
                    ...
                    print("MPC solver failed to find a solution, falling back to geometric controller")
                    return self.geometric_fallback(current_state, reference_trajectory[0])
            ...
            print(f"Unexpected error in MPC solve: {e}")
            traceback.print_exc()
            return self.geometric_fallback(current_state, reference_trajectory[0])
    ...
    def _validate_inputs(self, current_state, reference_trajectory):
        """Validate input data"""
        try:
            if current_state.shape[0] != 4:
                return False
            if not isinstance(reference_trajectory, (np.ndarray, list)):
                return False
            return True
        except:
            return False

    def _process_reference_trajectory(self, reference_trajectory):
    ...
    def _setup_variable_bounds(self):
        """Set up variable bounds for the optimization"""
    ...
    def _setup_constraint_bounds(self):
        """Set up constraint bounds for the optimization"""
    ...
    def geometric_fallback(self, current_state, target):
        """Geometric controller for emergency situations"""
        ...
        # Steering control
        steer = np.clip(yaw_error * 1.2, -0.785, 0.785)
        
        # Speed control
        dist = np.hypot(dx, dy)
        throttle = np.clip(dist * 0.3, 0.0, 1.0)
...
class GlobalPathPlanner:
    ...
    def a_star_search(self, start, goal):
        ...
        # Generate dense path with intermediate points
        dense_path = []
        for i in range(len(path)-1):
            start_pt = path[i].transform.location
            end_pt = path[i+1].transform.location
            num_segments = int(start_pt.distance(end_pt) // 2.0)

            for t in np.linspace(0, 1, num_segments + 2)[:-1]:
                new_loc = start_pt + t*(end_pt - start_pt)
                dense_path.append(self.world.get_map().get_waypoint(new_loc))
...
class CarEnv:
    ...
    def get_reference_from_global_path(self, current_state):
        """Generate smoother reference trajectory with curvature-based speed"""
        ref_traj = []
        current_pos = np.array([current_state[0], current_state[1]])
        
        # Find closest waypoint with lookahead
        closest_idx = max(0, self.current_path_index - 5)
        for i in range(closest_idx, len(self.global_path)):
            if i > self.current_path_index + 15:  # 15 point lookahead
                break
            wp = self.global_path[i]
            
            # Calculate curvature-based speed
            curvature = self.calculate_curvature(i)
            speed = min(12.0, 8.0 / (1.0 + 5.0*abs(curvature)))
            
            ref_traj.append([
                wp.transform.location.x,
                wp.transform.location.y,
                np.radians(wp.transform.rotation.yaw),
                speed
            ])
        
        # Interpolate for smoother trajectory
        return self.interpolate_trajectory(ref_traj, self.mpc.N)

    def calculate_curvature(self, idx):
        """Calculate curvature using three consecutive points"""
        if idx < 1 or idx >= len(self.global_path)-1:
            return 0.0
            
        prev_pt = self.global_path[idx-1].transform.location
        curr_pt = self.global_path[idx].transform.location
        next_pt = self.global_path[idx+1].transform.location
        ...
        return (dx1*dy2 - dx2*dy1) / ((dx1**2 + dy1**2) * (dx2**2 + dy2**2))**0.5

    def interpolate_trajectory(self, path, num_points):
        ...
        return np.column_stack([
            ...
            sy(new_t), 
            syaw(new_t), 
            np.full(num_points, 8.0)  # Base speed
        ])
